	...
	std::vector<Matrix4f>& p_global)
{
	...
	// p_global(i) = p_global(parent(i)) * p_offset(i) * p_local(i)
	p_global.at(0) = p_offset.at(0) * p_local.at(0); // root has no parent, multiply by identity matrix instead
	for (idx = 1; idx < p_numJoints; idx++)
	{
		p_global.at(idx) = p_global.at(p_jointParent.at(idx)) *
			p_offset.at(idx) *
			p_local.at(idx);
	}
}
		...
		std::vector<Vector3f>& p_deformedVertices)
{
	...
	Vector4f homog_vtx;
	Vector4f homog_vtx_prime;
	float weight;

	for (vtx_idx = 0; vtx_idx < p_vertices.size(); vtx_idx++)
	{
		homog_vtx_prime << 0.0, 0.0, 0.0, 0.0; // init v' to 0-vector
		homog_vtx = toHomog(p_vertices.at(vtx_idx));
		for (joint_idx = 0; joint_idx < p_numJoints; joint_idx++)
		{
			// v' += w(j,v) * F(j) * A^-1(j) * v
			// formula implemented directly from lecture 11 slides
			weight = p_weights[joint_idx][vtx_idx];
			if (weight == 0)
			{
				continue;
			}
			...
			{
				homog_vtx_prime += weight *
					p_jointTrans[joint_idx] *
					p_jointTransRestInv[joint_idx] *
					homog_vtx;
			}
		}
		p_deformedVertices.at(vtx_idx) = fromHomog(homog_vtx_prime);
	}
